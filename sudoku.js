/* https://jquerypluginsfree.com/8-free-javascript-sudoku-game-code-examples/ */
...
class Sudoku {
	...
	boardGenerator (n, fixCellsNr) {
		...
		for (let i = 0; i < n*n; i++) {
			for (let j = 0; j < n*n; j++) {
				let value = Math.floor( (i*n + i/n + j) % (n*n) + 1 );
		...
		let bands_horizontal_indexes = this.shuffle(blank_indexes);
		...
		let bands_vertical_indexes = this.shuffle(blank_indexes);
		board_solution_tmp = [];
		index = 0;
		...
		for (let i=0; i< board_indexes.length; i++) {
	...
	drawBoard () {
		...
		let sudoku_board = $('<div></div>').addClass('sudoku_board');
		let sudoku_statistics = $('<div></div>')
				.addClass('statistics')
				.html('<b>Cells:</b> <span class="cells_complete">'
					+ this.cellsComplete +'/'+this.cellsNr +'</span> <b>Time:</b> <span class="time">'
					+ this.secondsElapsed + '</span>');
		...
		for (let i=0; i < this.nn; i++) {
			for (let j=0; j < this.nn; j++) {
				...
				let value = (this.board[index] > 0 ? this.board[index] : ''),
		...
		// draw console
		let sudoku_console_cotainer = $('<div></div>').addClass('board_console_container');
		let sudoku_console = $('<div></div>').addClass('board_console');
		...
		// draw gameover
		let sudoku_gameover = $('<div class="gameover_container"><div class="gameover">Congratulation! <button class="restart">Play Again</button></div></div>');
	...
	resizeWindow () {
		...
		let screen = { w: $(window).width(), h: $(window).height() };

		// adjust the board
		let	b_pos = $('#'+ this.id +' .sudoku_board').offset(),
		...
		let screen_wr = screen.w + s_dim.h + b_pos.top + 10;
		...
		let cell_width = $('#'+ this.id +' .sudoku_board .cell:first').width(),
		...
		// adjust the console
		let console_cell_width = $('#'+ this.id +' .board_console .num:first').width();
		...
		let c_dim = { w: $('#'+ this.id +' .board_console').width(), h: $('#'+ this.id +' .board_console').height() };
		let c_pos_new = { left : ( b_dim.w/2 - c_dim.w/2 + b_pos.left ), top  : ( b_dim.h/2 - c_dim.h/2 + b_pos.top ) };
		$('#'+ this.id +' .board_console').css({'left': c_pos_new.left, 'top': c_pos_new.top});

		//adjust the gameover container
		let gameover_pos_new = { left : ( screen.w/20 ), top  : ( screen.w/20 + b_pos.top ) };
	...
	addValue (value) {
		...
		let	position = { x: $(this.cell).attr('x'), y: $(this.cell).attr('y') },
			group_position = { x: Math.floor((position.x -1)/3), y: Math.floor((position.y-1)/3) },
	...
	run () {
		...
		$('#'+ this.id +' .board_console .num').on('click', function(e) {
			let	value  = $.isNumeric($(this).text()) ? parseInt($(this).text()) : 0,
